...
const FieldLabel = ({ htmlFor, children, required }: { htmlFor?: string; children: React.ReactNode; required?: boolean }) => (
  <label htmlFor={htmlFor} className="flex items-center text-sm font-medium text-gray-700 gap-1">
    <span>{children}</span>
      ...
      <DialogContent className="sm:max-w-[425px]">
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="space-y-2">
            ...
            <Input id="title" required error={errors.title?.message} {...register('title')} />
          </div>
          ...
          <div className="space-y-2">
            <FieldLabel htmlFor="fileUrl" required>
              File URL (Demo)
            </FieldLabel>
            <Input
              id="fileUrl"
