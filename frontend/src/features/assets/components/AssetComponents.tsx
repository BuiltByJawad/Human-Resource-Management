...
import { Button, Select, Input, TextArea, DatePicker } from '@/components/ui/FormComponents'
...
export const AssetCard = ({ asset, onAssign, onReturn, onEdit }: AssetCardProps) => {
  const currentAssignment = (Array.isArray(asset.assignments) ? asset.assignments : []).find(
    (a: AssetAssignment) => !a.returnedDate,
  )
...
interface MaintenanceModalProps {
  isOpen: boolean
  onClose: () => void
  onSave: (log: MaintenanceLogInput) => Promise<void>
  asset?: Asset | null
}

const maintenanceSchema = yup.object().shape({
  description: yup.string().required('Description is required'),
  cost: yup.number().nullable().transform((value, originalValue) => (originalValue === '' ? null : value)).notRequired(),
  date: yup.string().required('Date is required'),
  performedBy: yup.string().notRequired(),
})

type MaintenanceLogInput = yup.InferType<typeof maintenanceSchema>

export const MaintenanceModal = ({ isOpen, onClose, onSave, asset }: MaintenanceModalProps) => {
  const [loading, setLoading] = useState(false)
  ...
  useEffect(() => {
    if (isOpen && asset) {
      reset({
        ...
        performedBy: asset.assignments?.[0]?.employee?.firstName
  ...
  const onFormSubmit = async (data: MaintenanceLogInput) => {
    setLoading(true)
    try {
      await onSave(data)
      onClose()
          ...
          <div className="flex min-h-full items-center justify-center p-4 text-center sm:p-0">
            <Dialog.Panel className="relative transform overflow-hidden rounded-lg bg-white px-4 pb-4 pt-5 text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
              ...
              <form onSubmit={handleSubmit(onFormSubmit)} className="space-y-4">
                ...
                <div>
                  <Input label="Performed By" error={errors.performedBy?.message} {...register('performedBy')} />
                </div>
                <div className="mt-5 sm:mt-6">
                  <Button type="submit" loading={loading} className="w-full">
