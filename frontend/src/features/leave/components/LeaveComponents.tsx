...
import { Button, Select, TextArea, DatePicker } from '@/components/ui/FormComponents'
...
export function LeaveRequestCard({
  ...
  const statusIcons: Record<LeaveStatus, typeof ClockIcon> = {
    pending: ClockIcon,
  ...
  return (
    <div className="bg-white rounded-lg shadow p-6 border border-gray-100 hover:shadow-md transition-shadow">
      ...
      <div className="space-y-2 text-sm text-gray-600 mb-4">
        ...
        {request.reason && (
          <div className="mt-2 p-2 bg-gray-50 rounded text-xs italic">&quot;{request.reason}&quot;</div>
        )}
      </div>
      ...
      {request.status !== 'pending' && request.approver && (
        <div className="mt-4 pt-4 border-t border-gray-100 text-xs text-gray-400 text-center">
          {request.status === 'approved' ? 'Approved' : 'Rejected'} by {request.approver.firstName}{' '}
